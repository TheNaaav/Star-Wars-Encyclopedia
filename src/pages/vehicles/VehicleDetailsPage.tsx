...
import type { Vehicle, LinkedResource } from "../../types/resources";
...
export default function VehicleDetailsPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        const vehicle = await getByUrl<Vehicle>(`${BASE}/vehicles/${id}/`);
        if (alive) setData(vehicle);
      } catch {
        if (alive) setError("Could not load vehicle.");
      } finally {
      ...
      <div className="details__card">
        ...
        <div className="kv">
          ...
          <div className="kv__row">
            ...
            <div className="kv__val">{data.vehicle_class}</div>
          </div>
        </div>
