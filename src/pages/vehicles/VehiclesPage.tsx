...
export default function VehiclesPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError("");
      try {
        const res = await getList<Vehicle>("vehicles", page, query);
    ...
    <div>
      ...
      <p className="hint">
        {query.trim()
          ...
          : `Showing all ${data.total} vehicles`}
      </p>

      <ResourceGrid>
        {(data.data || []).map((v) => (
          <VehicleCard key={v.url} vehicle={v} />
        ))}
      </ResourceGrid>
...
function VehicleCard({ vehicle }: { vehicle: Vehicle }) {
  const id = vehicle.id || extractIdFromUrl(vehicle.url || "");

  return (
    <article className="card">
      <div className="card__header">{vehicle.name}</div>
      <div className="card__body">
        ...
        <Link className="card__btn" to={`/vehicles/${id}`}>
          Read more
        </Link>
      </div>
    </article>
